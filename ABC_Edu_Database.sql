...
DROP DATABASE IF EXISTS `ABC_Edu`;
CREATE DATABASE `ABC_Edu`; 
USE `ABC_Edu`;
...
BEGIN
    ...
    START TRANSACTION;
    -- Check if student already enrolled in unit
	...
	CASE
		WHEN stu_action = 'Register unit' THEN
			-- Prevent student from re-enrolling in enrolled unit
            IF var_enrolled_unit > 0 THEN
				SIGNAL SQLSTATE '45000'
				SET MESSAGE_TEXT = 'Unit already enrolled.';
			END IF;
            ...
            IF var_is_advanced_unit = 'YES' THEN
				INSERT INTO Fee (student_id, semester, fee_amount, status, description) VALUES
				(stu_student_id, stu_semester, 4000.00, 'UNPAID', 'Tuition Fee');
			ELSE 
				INSERT INTO Fee (student_id, semester, fee_amount, status, description) VALUES
				(stu_student_id, stu_semester, 3000.00, 'UNPAID', 'Tuition Fee');
			END IF;
		ELSE BEGIN END;
	END CASE;
    CASE
		WHEN stu_action = 'Drop unit' THEN
            IF var_enrolled_unit > 0 THEN
				...
				IF var_current_date > var_drop_deadline THEN
                    UPDATE Registration
					SET dropped_out_date = var_current_date, status = 'dropped_with_penalty'
                    WHERE student_id = stu_student_id AND unit_code = stu_unit_id AND semester = stu_semester;
                    
                    UPDATE Fee
					SET fee_amount = fee_amount + var_penalty_amount, status = 'UNPAID', description = 'Dropped Unit Penalty'
                    WHERE student_id = stu_student_id AND semester = stu_semester;
				-- If current date is before drop deadline, update record in Registration table
				ELSE
					UPDATE Registration
					SET dropped_out_date = var_current_date, status = 'dropped_without_penalty'
                    WHERE student_id = stu_student_id AND unit_code = stu_unit_id AND semester = stu_semester;
                    
                    UPDATE Fee
					SET status = 'PAID', description = 'Dropped Unit Without Penalty'
                    WHERE student_id = stu_student_id AND semester = stu_semester;
				END IF;
			ELSE 
				-- Return error message when a student tries to drop an unit he/she did not enrol in
				SIGNAL SQLSTATE '45000'
...
(10022, '2024S1', 4600.00, 'UNPAID', 'Dropped Unit Penalty'),
(10023, '2024S1', 3000.00, 'PAID', 'Tuition Fee'),
(10023, '2024S2', 3000.00, 'UNPAID', 'Tuition Fee');
